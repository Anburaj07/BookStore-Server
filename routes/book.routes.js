...
bookRouter.get("/:id", async (req, res) => {
  ...
  try {
    ...
    if (!book) {
      res.status(404).json({ msg: `Book with ${id} not found!` });
    } else {
...
bookRouter.put("/update/:id", auth, async (req, res) => {
  ...
  try {
    ...
    if (!book) {
      res.status(404).json({ msg: `Book with ${id} is not found!` });
      return;
    }
    ...
    if (Title && Author && ISBN && Description && PublishedDate) {
      ...
      res.status(200).json({ msg: `Book with ${id} updated` });
    } else {
      res
        .status(400)
...
bookRouter.delete("/delete/:id", auth, async (req, res) => {
  try {
    ...
    if (!book) {
      ...
      res.status(200).json({ msg: `Book with ${id} deleted!` });
    }
  } catch (error) {
    console.log("error:", error);
    res.status(400).json({ error });
